...
#import data
import data
from data_bert import BertTokenizedCorpus
...
log_file_loss_val = open('log_file_loss_val_emb.txt','w') 
log_file_loss_train = open('log_file_loss_train_emb.txt','w') 
...
if args.resume:
    ...
    bert_net = BertForMaskedLM.from_pretrained('bert-base-cased')
    model_gen.embeddings.weight = bert_net.bert.embeddings.word_embeddings.weight
    model_gen.embeddings.requires_grad_(False)
...
def convert_idx_to_words(idx):
    ...
    for i in range(0,idx.size(1)):
        ...
        for j in range(0,idx.size(0)):
            sent_list.append(corpus.tokenizer.decode[idx[j,i]])
