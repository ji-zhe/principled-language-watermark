...
def synchronicity_test(index, local_context):
    ...
    for k in range(len(mask_candidates)):
        ...
        #                                                  topk=topk)
        k_context = [index for index in local_context]
        k_context[-2] = mask_candidates[k]['token']
        mask_candidates_k = generate_substitute_candidates(k_context,topk=topk)
...
def generate_substitute_candidates(text_processed, topk=2):
    global _calls_to_lm
    # print('generating:',text_processed) #
