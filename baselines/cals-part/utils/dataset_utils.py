...
def get_dataset(dtype):
    if dtype == "imdb":
        ...
        corpus = [t.replace(' @-@ ','-').replace(' @,@ ',',').replace(' @.@ ','.') for t in corpus if not t.strip().startswith("=") and len(t.strip())> 0]
        test_corpus = [t.replace(' @-@ ','-').replace(' @,@ ',',').replace(' @.@ ','.') for t in test_corpus if not t.strip().startswith("=") and len(t.strip()) > 0]
        train_num_sample = len(corpus)
        test_num_sample = len(corpus)
    elif dtype == "wikitextpure":
        corpus = load_dataset('wikitext', 'wikitext-2-raw-v1')['train']['text']
        test_corpus = load_dataset('wikitext', 'wikitext-2-raw-v1')['test']['text']
        ...
        train_num_sample = len(corpus)
        test_num_sample = len(corpus)
    elif dtype == "wikitext2":
        with open('data/wikitext-2/test.txt','r') as ftest, open('data/wikitext-2/train.txt','r') as ftrain:
          corpus = ftrain.readlines()
          test_corpus = ftest.readlines()
        corpus = [t.replace(' @-@ ','-').replace(' @,@ ',',').replace(' @.@ ','.') for t in corpus if not t.strip().startswith("=") and len(t.strip())> 0]
        test_corpus = [t.replace(' @-@ ','-').replace(' @,@ ',',').replace(' @.@ ','.') for t in test_corpus if not t.strip().startswith("=") and len(t.strip()) > 0]
        train_num_sample = len(corpus)
        test_num_sample = len(corpus)
    elif dtype == "wikitext2debug":
        with open('data/wikitext-2-debug/test.txt','r') as ftest, open('data/wikitext-2-debug/train.txt','r') as ftrain:
          corpus = ftrain.readlines()
          test_corpus = ftest.readlines()
        corpus = [t.replace(' @-@ ','-').replace(' @,@ ',',').replace(' @.@ ','.') for t in corpus if not t.strip().startswith("=") and len(t.strip())> 0]
        test_corpus = [t.replace(' @-@ ','-').replace(' @,@ ',',').replace(' @.@ ','.') for t in test_corpus if not t.strip().startswith("=") and len(t.strip()) > 0]
