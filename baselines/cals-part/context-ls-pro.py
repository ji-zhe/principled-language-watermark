...
if __name__ == "__main__":
    ...
    cover_texts = preprocess2sentence(cover_texts, dtype+"-test", start_sample_idx, num_sample2load['test'],
                                      spacy_model=args.spacy_model,use_cache=False)
    ...
    if args.embed:
        ...
        for c_idx, sentences in enumerate(tqdm(cover_texts)):
            for sen_idx, sen in enumerate(sentences):
                sen_text = sen.text 
                if assertion_wt2:
                    sen_text = sen_text.replace('<unk>','[UNK]')
                logger.info(sen)
    ...
    if args.extract:
        ...
        if corrupted_flag:
            with open(corrupted_dir, "r") as reader:
                for line in reader:
                    line = line.strip('\n ')
                    items = line.split('\t')
                    corrupted_watermarked.append(items[-3])
        ...
        for idx, (c_idx, sen_idx, sub_idset, sub_idx, clean_wm_sen, key, msg) in enumerate(clean_watermarked):
            ...
            if (len(corrupted_watermarked) > 0 and len(corrupted_watermarked) <= idx) or idx >= args.num_sample:
                logger.info(f"Extracted all corrupted watermarks. Ending extraction... ")
                break

            original_sentences = cover_texts[c_idx]
            sen = original_sentences[sen_idx].text
            if assertion_wt2:
                sen = sen.replace('<unk>','[UNK]')
            ...
            for wm_text in wm_texts:
                ...
                logger.info(f"Watermarked sentence: {clean_wm_sen}")
