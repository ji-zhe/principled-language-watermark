...
parser.add_argument('--eval_valid', type=int, default=1,
                    help='write metric files')
parser.add_argument('--do_entail', type=int, default=1,
                    help='do_entail')
    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        with torch.no_grad():
            ...
            for beam in range(0,args.samples_num):
                ...
                meteor_adv = meteor_score([output_list],orig_list)
                spacy_meteor_adv = meteor_score([[token.text for token in spacynlp(orig_text)]],
                                                [token.text for token in spacynlp(output_text)])
                meteor_beams.append(meteor_adv)
                spacy_meteor_beams.append(spacy_meteor_adv)
                ...
                if args.do_entail:
                    ...
                    for item in es_res[0]:
                        if item['label'] == 'ENTAILMENT':
                            entailment_score_beams.append(item['score'])
                            break
                    else:
                        entailment_score_beams.append(1)
            ...
            msg_out,_ = model_gen.forward_msg_decode(best_beam_emb)
            
            fake_out = model_disc(candidates_emb[best_beam_idx].detach(), labels_gt)
            ...
            if bert_diff[best_beam_idx] < args.bert_threshold:
                ...
                spacy_meteor_tot = spacy_meteor_tot + spacy_meteor_beams[best_beam_idx]
                ...
                spacy_meteor_pair = spacy_meteor_beams[best_beam_idx]
                entailment_score = entailment_score_beams[best_beam_idx]
                out_file.write(str(meteor_pair)+' '+str(spacy_meteor_pair)+'\n')
                out_file.write(str(bert_diff[best_beam_idx]) +'\n')
                out_file.write(str(entailment_score)+'\n')
                out_file.write(str(ss_score[best_beam_idx])+'\n')
                ...
                if args.count_token:
                    ...
                    f_token.write(' '.join(output_list)+'\n')
            else:
                spacy_meteor_pair, meteor_pair = 1, 1
                ss_pair = 1
...
if args.cuda:
    ...
    model_disc.cuda()
if args.eval_valid:
    f = open('evaluate_'+args.outfile+'_valid.txt','w')
    if args.count_metric:
        f_metrics = open('evaluate_'+args.outfile+'_valid_metrics.txt','w')
    if args.count_token:
        f_token = open('evaluate_'+args.outfile+'_valid_ct.txt','w')
    val_lm_loss, val_correct_msg, val_correct_bits_msg,val_meteor,val_spacy_meteor, val_l2_sbert, val_correct_fake, val_correct_real, val_Fscore, val_pvalue, val_pvalue_inst, val_entailment, val_semantic_similarity = evaluate(val_data, f, eval_batch_size)

    print('-' * 150)
    print('| validation | lm loss {:5.2f} | msg accuracy {:5.2f} | msg bit accuracy {:5.2f} | meteor {:5.4f} | spacy meteor {:5.4f} | SentBert dist. {:5.4f} | entailment score: {:5.4f} | semantic similarity: {:5.4f} | fake accuracy {:5.2f} | real accuracy {:5.2f} | F1 score {:.5f} | P-value {:.9f} | P-value inst {:5.2f}'.format(val_lm_loss,val_correct_msg*100, val_correct_bits_msg*100, val_meteor, val_spacy_meteor, val_l2_sbert, val_entailment, val_semantic_similarity, val_correct_fake*100,val_correct_real*100,val_Fscore,val_pvalue,val_pvalue_inst*100))
    print('-' * 150)
    f.close()
    if args.count_metric:
        f_metrics.close()
...
# Run on test data.
f = open('evaluate_'+args.outfile+'_test.txt','w')
if args.count_metric:
    f_metrics = open('evaluate_'+args.outfile+'_test_metrics.txt','w')
if args.count_token:
    f_token = open('evaluate_'+args.outfile+'_test_ct.txt','w')
test_lm_loss, test_correct_msg, test_correct_bits_msg, test_meteor, test_spacy_meteor, test_l2_sbert, test_correct_fake, test_correct_real, test_Fscore, test_pvalue, test_pvalue_inst, test_entailment, test_semantic_similarity = evaluate(test_data, f, test_batch_size)

print('=' * 150)
print('| test | lm loss {:5.2f} | msg accuracy {:5.2f} | msg bit accuracy {:5.2f} | meteor {:5.4f} | spacy meteor {:5.4f} | SentBert dist. {:5.4f} | entailment score: {:5.4f} | semantic similarity: {:5.4f} | fake accuracy {:5.2f} | real accuracy {:5.2f} | F1 score {:.5f} | P-value {:.9f} | P-value inst {:5.2f}'.format(test_lm_loss, test_correct_msg*100, test_correct_bits_msg*100, test_meteor, test_spacy_meteor, test_l2_sbert, test_entailment, test_semantic_similarity, test_correct_fake*100,test_correct_real*100,test_Fscore,test_pvalue, test_pvalue_inst*100))
print('=' * 150)
f.close()
if args.count_metric:
    f_metrics.close()
