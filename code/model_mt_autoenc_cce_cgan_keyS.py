...
class TranslatorGeneratorModel(nn.Module):
    ...
    def __init__(self, ntoken, ninp, msg_len=64, msg_in_mlp_layers = 1, msg_in_mlp_nodes= [], nlayers_encoder=6, transformer_drop=0.1, dropouti=0.15, dropoute=0.1, tie_weights=False, shared_encoder=False, attention_heads=8, pretrained_model=None):
        ...
        self.msg_label_out_mlp = [nn.Linear(2*ninp, 2**msg_len)]
        self.msg_label_out_mlp = torch.nn.ModuleList(self.msg_label_out_mlp)
    ...
    def forward_msg_decode(self, fake, orig):
        # import pdb;pdb.set_trace()
        orig = torch.cat([orig, fake[orig.size(0):]], dim=0)
        input = torch.cat([fake,orig], dim=2)
        input = self.fc(input)
        # input = self.fc(input)
        # input = fake
        ...
        # plan1: concatenate(Y, S) (emb 1024) ---> nn.linear ---> emb 512
        # plan2: input = Y-S 
        ...
        msg_decoder_rep_copy = torch.cat([msg_dec_avg,last_state],dim=1)
        ...
        msg_decoder_rep = msg_decoder_rep_copy
...
class TranslatorDiscriminatorModel(nn.Module):
    def __init__(self, ninp, nlayers_encoder=6, transformer_drop=0.1, attention_heads=8, dropouti=0.1, msg_len=4):
        ...
        self.real_fake_classify = [nn.Linear(ninp*3,ninp), nn.Linear(ninp,1)]
        self.real_fake_classify = torch.nn.ModuleList(self.real_fake_classify)

        self.label_fc = nn.Linear(2**msg_len, ninp)
    ...
    def forward(self, input_emb, labels):
        #print(input_emb)
        ...
        labels = self.label_fc(labels)

        disc_enc_rep = torch.cat([disc_avg,last_state,labels],dim=1)
        for ff in self.real_fake_classify:       		
