...
from utils import batchify, get_batch_different, generate_msgs, repackage_hidden, freeze, unfreeze, get_batch_different_cgan
...
#does trunc corpus
parser.add_argument('--trunc_corpus', type=int, default=0,
                    help='path of corpus to resume')
...
def model_load(fn):
    global model_gen, criterion, criterion_reconst, criterion_label, optimizer_gen
...
if args.trunc_corpus:
    atk_train = corpus.train[:2000000]
    # atk_val = corpus.valid[:200000]
    atk_test = corpus.test[:200000]
else:
    atk_train = corpus.train
    # atk_val = corpus.valid
    atk_test = corpus.test
    
# It's noteworthy that train_adaptive_Attacker do truncation in 2,000,000 200,000 200,000.
...
for i in range(args.epochs):
    ...
    print('saving model')
    if (i+1)%10 == 0:
        model_save(args.save+'e'+str(i+1))
