...
import data
import model_mt_autoenc_cce_cgan
import lang_model
...
from utils import batchify, repackage_hidden, get_batch_different_cgan, generate_msgs
...
parser.add_argument('--atk_dist_weight', type=float, default=1.0,
                    help='dist weight in L_atk = L_msg + dist_weight * L_dist. L_dist=CE currently.')
parser.add_argument('--atk_atk_weight', type=float, default=0.1,
                    help='weight of loss_atk in L = loss_atk + loss_dist')
...
parser.add_argument('--outfile', type=str, default='results.txt',
                    help='output file')
parser.add_argument('--count_token', action='store_true',
                    help='write count token files')
parser.add_argument('--count_metric', action='store_true',
                    help='write metric files')
parser.add_argument('--eval_valid', type=int, default=1,
                    help='write metric files')
					
bert_masklm = BertForMaskedLM.from_pretrained('bert-base-cased').cuda()
...
def forward_sent_encoder_inputemb(self, input,msg_input, gumbel_temp, only_embedding=False):
    if only_embedding:
        emb = self.embeddings(input) 
        return emb

    emb = input * math.sqrt(self.ninp)
...
@torch.enable_grad()
def optimizing_attack(fake, batch_size, model_gen, model_atk, n_steps, dist_weight=1.0, atk_weight=0.1, lr=1e-4):
    with torch.no_grad():
        atk_init = autoenc_greedy(fake, batch_size)


    fake_emb = model_gen.forward_sent(fake,None,args.gumbel_temp,only_embedding=True).detach()
    fake_emb.requires_grad=False

    atk_init_hot = F.one_hot(atk_init, corpus.dictlen)
    atk_prob = atk_init_hot.to(torch.float32).detach().float().cuda()
    atk_prob.requires_grad = True
    optimizer = torch.optim.Adam([atk_prob])
    reconst_loss_fn = torch.nn.CrossEntropyLoss()
    dist_loss_fn = torch.nn.NLLLoss()
    # msg_atk = torch.zeros((batch_size, args.msg_len), dtype=torch.float).cuda()
    with torch.no_grad():
        msg_atk = model_gen.forward_msg_decode(fake_emb) 
    for _ in range(n_steps):
        # optimizer.zero_grad()
        # import pdb; pdb.set_trace()
        with torch.no_grad():
            # import pdb;pdb.set_trace()
            atk_prob = torch.clip(atk_prob, 1e-9, 1.0)
            atk_prob = atk_prob/(atk_prob.sum(dim=-1, keepdim=True))
        atk_prob.requires_grad=True
        atk_emb = torch.matmul(atk_prob, model_gen.embeddings.weight)
        cur_both_embeddings, cur_sent_encoder_out = forward_sent_encoder_inputemb(model_gen,atk_emb,msg_atk,args.gumbel_temp)
        sent_out_emb, sent_out_hot, sent_out_soft = model_gen.forward_sent_decoder(cur_both_embeddings, fake, cur_sent_encoder_out, args.gumbel_temp)

        loss_atk = reconst_loss_fn(sent_out_soft, fake.view(-1)) # Y, i.e. atk_emb, 要和X靠近，满足distortion constraint
        loss_dist = -torch.log(torch.gather(atk_prob, 2, fake.unsqueeze(dim=-1))).mean()#CrossEntropyLoss_fn(atk_prob, 
        # import pdb;pdb.set_trace()
        # print(loss_dist)
        total = atk_weight* loss_atk + dist_weight* loss_dist
        total.backward()
        # optimizer.step()
        atk_prob.data -= lr*atk_prob.grad
        atk_prob.grad=None
        # del loss_atk
        # del loss_dist
    print(loss_atk.item(), loss_dist.item())
    atk_idx = model_gen.decoder(atk_emb).argmax(dim=-1)
    return atk_idx, (msg_atk>0.5).float()
...
def evaluate(data_source, out_file, batch_size=10, on_train=False):
    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        with torch.no_grad():
            ...
            for beam in range(0,args.samples_num):
                ...
                output_text = corpus.tokenizer.decode(word_idx[:,0])
                orig_text = corpus.tokenizer.decode(data[:,0])
                if orig_text == output_text:
                    continue				
                output_list = []
                orig_list = []
                for k in range(0, data.size(0)):
                    output_list.append(corpus.dictionary.idx2word[word_idx[k,0]])
                    orig_list.append(corpus.dictionary.idx2word[data[k,0]])
                output_text_beams.append(output_text)
                output_list_beams.append(output_list)
            ...
            # Not Used meteor_score_selected = meteor_score([orig_text.split(' ')],output_text_beams[best_beam_idx].split(' '))
			
            best_beam_data = word_idx_beams[best_beam_idx]
            # data_adv_autoenc = autoenc_greedy(best_beam_data, batch_size)
            print(i)			
            data_adv_autoenc, msg_atk = optimizing_attack(best_beam_data,batch_size,model_gen,model_atk=model_autoenc_attack, n_steps=500,dist_weight=args.atk_dist_weight, atk_weight=args.atk_atk_weight)
            ...
            # data_dv_autoenc_one_hot = F.one_hot(data_adv_autoenc, corpus.dictlen).to(torch.float32)
            # mask = data != 	best_beam_data	
            ...
            if bert_diff_selected < args.bert_threshold:
                ...
                output_text_adv = corpus.tokenizer.decode(word_idx_adv[:,0])
                output_list_adv = []
                for k in range(0, data.size(0)):
                    output_list_adv.append(corpus.dictionary.idx2word[word_idx_adv[k,0]])
                sentences = [output_text_adv, orig_text]
                sbert_embs = sbert_model.encode(sentences)
                meteor_adv = meteor_score([output_list_adv],orig_list)
                spacy_meteor_adv = meteor_score([[token.text for token in spacynlp(orig_text)]],
                                                [token.text for token in spacynlp(output_text_adv)])
                ...
                spacy_meteor_tot = spacy_meteor_tot + spacy_meteor_adv
                out_file.write('****'+'\n')
                ...
                if args.count_token:
                    f_token.write(' '.join(orig_list)+'\n')
                    f_token.write(' '.join(output_list_adv)+'\n')
                    f_token.write(' '.join(output_list_beams[best_beam_idx])+'\n')
            else:
                meteor_pair = 1
                meteor_tot = meteor_tot + meteor_pair
                spacy_meteor_tot = spacy_meteor_tot + meteor_pair
    ...
    return total_loss_lm/batch_count, correct_msg_count/tot_count, correct_msg_count_bits/tot_count_bits, meteor_tot/batch_count, spacy_meteor_tot/batch_count, l2_distances/batch_count, fake_correct/batch_count, real_correct/batch_count, Fscore ,  np.mean(p_value), p_value_smaller/len(p_value)
...
with open(args.gen_path, 'rb') as f:	
    model_gen= torch.load(f)[0]
    
#print(model_gen)

with open(args.disc_path, 'rb') as f:
    model_disc= torch.load(f)[0]
#print(model_gen)


with open(args.autoenc_attack_path, 'rb') as f:
    model_autoenc_attack= torch.load(f)[0]
...
if args.eval_valid:
    f = open('evaluate_dae_'+args.outfile+'_valid.txt','w')
    if args.count_metric:
        f_metrics = open('evaluate_optimizingAtkEnc_'+args.outfile+'_valid_metrics.txt','w')
    if args.count_token:
        f_token = open('evaluate_optimizingAtkEnc_'+args.outfile+'_valid_ct.txt','w')
    val_lm_loss, val_correct_msg, val_correct_bits_msg,val_meteor,val_spacy_meteor, val_l2_sbert, val_correct_fake, val_correct_real, val_Fscore, val_pvalue, val_pvalue_inst = evaluate(val_data, f, eval_batch_size)

    print('-' * 150)
    print('| validation | lm loss {:5.2f} | msg accuracy {:5.2f} | msg bit accuracy {:5.2f} | meteor {:5.4f} | spacy meteor {:5.4f} | SentBert dist. {:5.4f} | fake accuracy {:5.2f} | real accuracy {:5.2f} | F1 score {:.5f} | P-value {:.9f} | P-value inst {:5.2f}'.format(val_lm_loss,val_correct_msg*100, val_correct_bits_msg*100, val_meteor, val_spacy_meteor, val_l2_sbert, val_correct_fake*100,val_correct_real*100,val_Fscore,val_pvalue,val_pvalue_inst*100))
    print('-' * 150)
    f.close()
    if args.count_metric:
        f_metrics.close()
...
# Run on test data.
f = open('evaluate_dae_'+args.outfile+'_test.txt','w')
if args.count_metric:
    f_metrics = open('evaluate_optimizingAtkEnc_'+args.outfile+'_test_metrics.txt','w')
if args.count_token:
    f_token = open('evaluate_optimizingAtkEnc_'+args.outfile+'_test_ct.txt','w')
test_lm_loss, test_correct_msg, test_correct_bits_msg, test_meteor, test_spacy_meteor, test_l2_sbert, test_correct_fake, test_correct_real, test_Fscore, test_pvalue, test_pvalue_inst = evaluate(test_data, f, test_batch_size)

print('=' * 150)
print('| test | lm loss {:5.2f} | msg accuracy {:5.2f} | msg bit accuracy {:5.2f} | meteor {:5.4f} | spacy meteor {:5.4f} | SentBert dist. {:5.4f} | fake accuracy {:5.2f} | real accuracy {:5.2f} | F1 score {:.5f} | P-value {:.9f} | P-value inst {:5.2f}'.format(test_lm_loss, test_correct_msg*100, test_correct_bits_msg*100, test_meteor, test_spacy_meteor, test_l2_sbert, test_correct_fake*100,test_correct_real*100,test_Fscore,test_pvalue, test_pvalue_inst*100))
print('=' * 150)
f.close()
if args.count_metric:
    f_metrics.close()
