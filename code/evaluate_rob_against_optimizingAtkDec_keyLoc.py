...
def forward_msg_decode_input_emb(self, fake_one_hot, mask, bert):
    # import pdb; pdb.set_trace()
    fake_ids = fake_one_hot.argmax(dim=-1) # indifferentiable
    orig_ids = fake_ids.clone()
    orig_ids[mask] = 103
    with torch.no_grad():
        bert_pred_logits = bert(orig_ids)[0]
    orig_one_hot = fake_one_hot.clone()
    orig_one_hot[mask] = F.one_hot(bert_pred_logits[mask].argmax(dim=-1),orig_one_hot.shape[-1]).to(torch.float32)
    # pred_ids = bert_pred_logits[diff.mask].argmax(dim=-1)
    orig_emb =  torch.matmul(orig_one_hot,self.embeddings.weight) # self.embeddings(orig_one_hot)
    fake_emb = torch.matmul(fake_one_hot, self.embeddings.weight)
    input = torch.cat([fake_emb,orig_emb], dim=2)
    input = self.fc(input)
    ...
    ### TODO: decoded_msg_label_out = ...
    for ff_label in self.msg_label_out_mlp:
        msg_decoder_rep = ff_label(msg_decoder_rep)

    decoded_msg_label_out = msg_decoder_rep

    return decoded_msg_out, decoded_msg_label_out

@torch.enable_grad()
def optimizing_attack(fake, batch_size, model_gen, n_steps, m_target=None, K_atk = None):
    if m_target is None:
        m_target = torch.randint(low=0,high=2,size=(batch_size, args.msg_len), dtype=torch.float).detach().cuda()
    if K_atk is None:
        K_atk = torch.zeros(fake.shape,dtype=torch.long)
        K_atk.bernoulli_(0.1)
    
    fake_emb = model_gen.forward_sent(fake,None,args.gumbel_temp,only_embedding=True).detach()
    fake_emb.requires_grad=False
    atk_emb = fake_emb.detach().clone()
    atk_emb.requires_grad=True
    optimizer = torch.optim.Adam([atk_emb])

    for _ in range(n_steps):
        optimizer.zero_grad()
        # import pdb; pdb.set_trace()
        m_hat, _  = model_gen.forward_msg_decoder()
        loss_atk = torch.norm(sent_out_emb - fake_emb, dim=-1).mean() # enc输出要与X靠近，从而找到S
        loss_dist = torch.norm(atk_emb - fake_emb, dim=-1).mean() # Y, i.e. atk_emb, 要和X靠近，满足distortion constraint
        # import pdb;pdb.set_trace()
        total = loss_atk + loss_dist
        total.backward()
        optimizer.step()
...
def evaluate(data_source, out_file, batch_size=10, on_train=False):
    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        with torch.no_grad():
            ...
            data_adv_autoenc, msg_atk = optimizing_attack(best_beam_data,batch_size,model_gen,n_steps=1000,dist_weight=0.05)
            ...
            keys = mask
            data_emb = model_gen.forward_sent(data,msgs,args.gumbel_temp,only_embedding=True)
            msg_out,_ = model_gen.forward_msg_decode(data_dv_autoenc_one_hot.detach(), keys, bert_masklm)
			
            #get prediction (and the loss) of the discriminator on the fake sequence.
            fake_out = model_disc(data_dv_autoenc_emb, labels_gt)
            ...
            if bert_diff_selected < args.bert_threshold:
                ...
                msg_out_random, _ =  model_gen.forward_msg_decode(data_emb, keys, bert_masklm)
...
with open(args.gen_path, 'rb') as f:	
    model_gen, _, _, _ , _= torch.load(f)	
    
#print(model_gen)

with open(args.disc_path, 'rb') as f:
    model_disc, _, _ , _, _= torch.load(f)
#print(model_gen)
...
if args.eval_valid:
    ...
    if args.count_metric:
        f_metrics = open('evaluate_optimizingAtkDec_'+args.outfile+'_valid_metrics.txt','w')
    if args.count_token:
        f_token = open('evaluate_optimizingAtkDec_'+args.outfile+'_valid_ct.txt','w')
...
if args.count_metric:
    f_metrics = open('evaluate_optimizingAtkDec_'+args.outfile+'_test_metrics.txt','w')
if args.count_token:
    f_token = open('evaluate_optimizingAtkDec_'+args.outfile+'_test_ct.txt','w')
