...
def evaluate(data_source, out_file, batch_size=10, on_train=False):
    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        with torch.no_grad():
            ...
            meteor_score_selected = meteor_score([orig_text.split(' ')],output_text_beams[best_beam_idx].split(' '))
            ...
            data_orig_autoenc = autoenc_greedy(data, batch_size)
            data_orig_autoenc_emb = model_gen.forward_sent(data_orig_autoenc, msgs, args.gumbel_temp, only_embedding=True)
            # print(data.shape)
            # print(best_beam_data.shape)
            # print(data_dv_autoenc_emb.shape)
            # print(data_emb.shape)
            msg_out = model_gen.forward_msg_decode(data_dv_autoenc_emb.detach(), data_orig_autoenc_emb.detach())
            ...
            if bert_diff_selected < args.bert_threshold:
                ...
                sbert_embs = sbert_model.encode(sentences)
                meteor_adv = meteor_score([orig_text.split(' ')],output_text_adv.split(' '))
                ...
                msg_out_random =  model_gen.forward_msg_decode(data_emb, data_emb)
