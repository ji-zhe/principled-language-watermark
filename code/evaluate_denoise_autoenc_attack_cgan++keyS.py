    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        with torch.no_grad():
            ...
            if bert_diff_selected < args.bert_threshold:
                ...
                # es_res = pipe_classification(output_text_adv+" </s></s> "+orig_text)
                # for item in es_res:
                #     if item['label'] == 'ENTAILMENT':
                #         entailment_score_adv=item['score']
                #         break
                    
                # ss_embs = ss_model.encode(sentences)
                # cosine_scores = cos_sim(ss_embs[0], ss_embs[1])
                # ss_score_adv=(torch.diagonal(cosine_scores).tolist())[0]
                ...
                # entailment_score_tot = entailment_score_tot + entailment_score_adv
                # ss_tot = ss_tot + ss_score_adv
                out_file.write('****'+'\n')
                out_file.write(str(batch_count)+'\n')
                meteor_pair = meteor_adv
                spacy_meteor_pair = meteor_adv
                # entailment_score_pair = entailment_score_adv
                # ss_score_pair = ss_score_adv
                ...
                meteor_pair = 1
                spacy_meteor_pair = 1
                # entailment_score_pair = 1
                # ss_score_pair = 1
                meteor_tot = meteor_tot + meteor_pair
                spacy_meteor_tot = spacy_meteor_tot + spacy_meteor_pair
                # entailment_score_tot = entailment_score_tot + entailment_score_pair
                # ss_tot = ss_tot + ss_score_pair
...
if args.eval_valid:
    ...
    if args.count_metric:
        f_metrics = open('evaluate_dae_'+args.outfile+'_valid_metrics.txt','w')
    if args.count_token:
        f_token = open('evaluate_dae_'+args.outfile+'_valid_ct.txt','w')
    val_lm_loss, val_correct_msg, val_correct_bits_msg,val_meteor,val_spacy_meteor, val_l2_sbert, val_correct_fake, val_correct_real, val_Fscore, val_pvalue, val_pvalue_inst, val_entailment, val_ss = evaluate(val_data, f, eval_batch_size)

    print('-' * 150)
    print('| validation | lm loss {:5.2f} | msg accuracy {:5.2f} | msg bit accuracy {:5.2f} | meteor {:5.4f} | spacy meteor {:5.4f} | SentBert dist. {:5.4f} | entailment score: {:5.4f} | semantic similarity: {:5.4f} | fake accuracy {:5.2f} | real accuracy {:5.2f} | F1 score {:.5f} | P-value {:.9f} | P-value inst {:5.2f}'.format(val_lm_loss,val_correct_msg*100, val_correct_bits_msg*100, val_meteor, val_spacy_meteor, val_l2_sbert, val_entailment, val_ss, val_correct_fake*100,val_correct_real*100,val_Fscore,val_pvalue,val_pvalue_inst*100))
    print('-' * 150)
    f.close()
    if args.count_metric:
        f_metrics.close()
...
if args.count_metric:
    f_metrics = open('evaluate_dae_'+args.outfile+'_test_metrics.txt','w')
if args.count_token:
    f_token = open('evaluate_dae_'+args.outfile+'_test_ct.txt','w')
test_lm_loss, test_correct_msg, test_correct_bits_msg, test_meteor, test_spacy_meteor, test_l2_sbert, test_correct_fake, test_correct_real, test_Fscore, test_pvalue, test_pvalue_inst, test_entailment, test_ss = evaluate(test_data, f, test_batch_size)

print('=' * 150)
print('| test | lm loss {:5.2f} | msg accuracy {:5.2f} | msg bit accuracy {:5.2f} | meteor {:5.4f} | spacy meteor {:5.4f} | SentBert dist. {:5.4f} | entailment score: {:5.4f} | semantic similarity: {:5.4f} | fake accuracy {:5.2f} | real accuracy {:5.2f} | F1 score {:.5f} | P-value {:.9f} | P-value inst {:5.2f}'.format(test_lm_loss, test_correct_msg*100, test_correct_bits_msg*100, test_meteor, test_spacy_meteor, test_l2_sbert, test_entailment, test_ss, test_correct_fake*100,test_correct_real*100,test_Fscore,test_pvalue, test_pvalue_inst*100))
print('=' * 150)
f.close()
if args.count_metric:
    f_metrics.close()
