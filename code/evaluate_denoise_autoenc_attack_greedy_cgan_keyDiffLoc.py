...
import data
import model_mt_autoenc_cce_cgan_keyDiffS as model_mt_autoenc_cce_cgan
import lang_model
...
#use saved corpus
parser.add_argument('--saved_corpus', type=str, default='/mnt/disk2/jizhe/zyc/awt/tokenizer_WT2.data',
...
def evaluate(data_source, out_file, batch_size=10, on_train=False):
    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        with torch.no_grad():
            ...
            best_beam_one_hot = F.one_hot(best_beam_data, (corpus.dictlen)).to(torch.float32)
            data_dv_autoenc_one_hot = F.one_hot(data_adv_autoenc, corpus.dictlen).to(torch.float32)
            mask = data != 	best_beam_data	
            keys = mask
            data_emb = model_gen.forward_sent(data,msgs,args.gumbel_temp,only_embedding=True)	
            msg_out, _ = model_gen.forward_msg_decode(data_dv_autoenc_one_hot, keys, bert_masklm)
            ...
            if bert_diff_selected < args.bert_threshold:
                ...
                data_one_hot = F.one_hot(data, corpus.dictlen).to(torch.float32)
                msg_out_random, _ =  model_gen.forward_msg_decode(data_one_hot, keys, bert_masklm)
