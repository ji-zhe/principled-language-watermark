...
def evaluate(data_source, out_file, batch_size=10, on_train=False):
    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        with torch.no_grad():
            ...
            best_beam_data_hot = F.one_hot(best_beam_data,candidates_soft_prob[0].size(-1)).to(torch.float32)
            best_beam_emb =  model_gen.forward_sent(best_beam_data,msgs,args.gumbel_temp,only_embedding=True)
            mask = data != 	best_beam_data	
            keys = mask
            data_emb = model_gen.forward_sent(data,msgs,args.gumbel_temp,only_embedding=True)	
            msg_out, _ = model_gen.forward_msg_decode(best_beam_data_hot, keys, bert_masklm)
            ...
            if bert_diff[best_beam_idx] < args.bert_threshold:
                ...
                data_one_hot = F.one_hot(data, candidates_soft_prob[0].size(-1)).to(torch.float32)
...
if args.eval_valid:
    ...
    val_lm_loss, val_correct_msg, val_correct_bits_msg,val_meteor,val_spacy_meteor, val_l2_sbert, val_correct_fake, val_correct_real, val_Fscore, val_pvalue, val_pvalue_inst, val_entailment = evaluate(val_data, f, eval_batch_size)

    print('-' * 150)
    print('| validation | lm loss {:5.2f} | msg accuracy {:5.2f} | msg bit accuracy {:5.2f} | meteor {:5.4f} | spacy meteor {:5.4f} | SentBert dist. {:5.4f} | fake accuracy {:5.2f} | real accuracy {:5.2f} | F1 score {:.5f} | P-value {:.9f} | P-value inst {:5.2f} | entailment score: {:5.2f}'.format(val_lm_loss,val_correct_msg*100, val_correct_bits_msg*100, val_meteor, val_spacy_meteor, val_l2_sbert, val_correct_fake*100,val_correct_real*100,val_Fscore,val_pvalue,val_pvalue_inst*100, val_entailment))
    print('-' * 150)
    f.close()
    if args.count_metric:
        f_metrics.close()
...
test_lm_loss, test_correct_msg, test_correct_bits_msg, test_meteor, test_spacy_meteor, test_l2_sbert, test_correct_fake, test_correct_real, test_Fscore, test_pvalue, test_pvalue_inst,test_entailment = evaluate(test_data, f, test_batch_size)

print('=' * 150)
print('| test | lm loss {:5.2f} | msg accuracy {:5.2f} | msg bit accuracy {:5.2f} | meteor {:5.4f} | spacy meteor {:5.4f} | SentBert dist. {:5.4f} | fake accuracy {:5.2f} | real accuracy {:5.2f} | F1 score {:.5f} | P-value {:.9f} | P-value inst {:5.2f} | entailment score: {:5.2f}'.format(test_lm_loss, test_correct_msg*100, test_correct_bits_msg*100, test_meteor, test_spacy_meteor, test_l2_sbert, test_correct_fake*100,test_correct_real*100,test_Fscore,test_pvalue, test_pvalue_inst*100, test_entailment))
print('=' * 150)
f.close()
if args.count_metric:
    f_metrics.close()
