main_train_cgan_kd_smine_keySimport argparse
import time
import math
import numpy as np
import torch
...
###
params = list(model_gen.parameters()) + list(criterion.parameters()) + list(criterion_reconst.parameters()) + list(model_atk.parameters()) + list(mine_us.parameters())+list(mine_uy.parameters())
...
def evaluate(data_source, batch_size=10):
    ...
    return total_loss_reconst.item()/batches_count, total_loss_msg.item()/batches_count, total_mine_us.item()/batches_count,total_mine_uy.item()/batches_count, total_loss_sem / batches_count, total_loss_lm/batches_count, total_loss_msg_1.item()/batches_count, total_loss_msg_2.item()/batches_count
...
def train():
	...
	#get the embeddings from the generator network of the real 
        ...
        mine_uy.optimize(fake_sent_msg_embedding.detach(), atk_data_emb.detach().permute((1,0,2)), iters=4, batch_size=fake_data_emb.size(1),opt=optimizer_mine_uy) ### : batch first! 
        mine_us.optimize(fake_sent_msg_embedding.detach(), data_emb.detach().permute((1,0,2)), iters=4, batch_size=fake_data_emb.size(1),opt=optimizer_mine_us) ### : batch first! 
        ...
        msg_out_clean = model_gen.forward_msg_decode(fake_data_emb)
        ...
        if args.use_semantic_loss: 
            ...
            errG = args.reconst_weight*errG_reconst + args.xsmi_weight * errG_mi + args.msg_weight*errG_msg + args.sem_weight*errG_sem 
            sem_losses.append(errG_sem.item())
            total_loss_sem += errG_sem.item()
        else:
            errG = args.reconst_weight*errG_reconst + args.xsmi_weight * errG_mi + args.msg_weight*errG_msg
        ...
        if batch % args.log_interval == 0 and batch > 0:
            ...
            print('| epoch {:3d} | {:5d}/{:5d} batches | gen lr {:05.5f} | ms/batch {:5.6f} | '
                    'msg loss {:5.6f}  | sem loss {:5.6f} | reconst loss {:5.6f} | lm loss {:5.6f} | errA_mi -- | errA_reconst {:5.6f}| msg_1 loss {:5.6f} | msg_2 loss {:5.6f} '.format(
                epoch, batch, len(train_data) // args.bptt, optimizer_gen.param_groups[0]['lr'],
                elapsed * 1000 / args.log_interval, cur_loss_msg, cur_loss_sem, cur_loss_reconst, cur_loss_lm, errA_reconst, cur_loss_msg_1, cur_loss_msg_2))
            total_loss_gen = 0
            total_loss_msg = 0
            total_loss_disc = 0
            total_loss_sem = 0
...
try:
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if 't0' in optimizer_gen.param_groups[0]:
            ...
            val_loss_reconst, val_loss_msg, val_loss_mine_us,val_loss_mine_uy, val_loss_sem, val_loss_lm, val_loss_msg_1, val_loss_msg_2 = evaluate(val_data, eval_batch_size)
            val_loss_gen_tot2 = args.reconst_weight * val_loss_reconst + args.msg_weight * val_loss_msg + args.xsmi_weight * (val_loss_mine_us - val_loss_mine_uy) + args.sem_weight * val_loss_sem + args.lm_weight * val_loss_lm

            print('-' * 89)
            print('| end of epoch {:3d} | time: {:5.6f}s | val_loss_reconst: {:5.6f} | val_loss_msg: {:5.6f} | val_loss_mine_us: {:5.6f} | val_loss_mine_uy: {:5.6f} | val_loss_sem: {:5.6f} | val_loss_lm: {:5.6f} | val_loss_msg_1: {:5.6f} | val_loss_msg_2: {:5.6f}'.format(
                    epoch, (time.time() - epoch_start_time),val_loss_reconst, val_loss_msg, val_loss_mine_us,val_loss_mine_uy, val_loss_sem, val_loss_lm, val_loss_msg_1, val_loss_msg_2))
            print('-' * 89)
            ...
            val_loss_reconst, val_loss_msg, val_loss_mine_us,val_loss_mine_uy, val_loss_sem, val_loss_lm,val_loss_msg_1, val_loss_msg_2 = evaluate(val_data, eval_batch_size)
            val_loss_gen_tot = args.reconst_weight * val_loss_reconst + args.msg_weight * val_loss_msg + args.xsmi_weight * (val_loss_mine_us - val_loss_mine_uy)+ args.sem_weight * val_loss_sem + args.lm_weight * val_loss_lm
...
# Run on test data.
test_loss_reconst, test_loss_msg, test_loss_mine_us,test_loss_mine_uy, test_loss_sem, test_loss_lm, test_loss_msg_1, test_loss_msg_2 = evaluate(test_data, test_batch_size)

print('-' * 89)
print('| end of training  {:3d} | time: {:5.6f}s | test_loss_reconst: {:5.6f} | test_loss_msg: {:5.6f} | test_loss_mine_us: {:5.6f} | test_loss_mine_uy: {:5.6f} | test_loss_sem: {:5.6f} | test_loss_lm: {:5.6f} | test_loss_msg_1: {:5.6f} | test_loss_msg_2: {:5.6f}'.format(
        epoch, (time.time() - epoch_start_time),test_loss_reconst, test_loss_msg, test_loss_mine_us,test_loss_mine_uy, test_loss_sem, test_loss_lm, test_loss_msg_1, test_loss_msg_2))
print('-' * 89)