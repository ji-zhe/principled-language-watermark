...
class AutoencModel(nn.Module):
    ...
    def forward_sent_encoder(self, input, is_embedding=False):
        if is_embedding:
        ...
        sent_embedding = sent_encoder_out.mean(dim=0)

        return sent_embedding, sent_encoder_out

    def forward_sent_decoder(self, sent_embedding, input_data, sent_encoder_out, gumbel_temp, is_embedding=False, return_feature = False):
        device = input_data.device
        mask = self._generate_square_subsequent_mask(len(input_data)).to(device)
        
        if is_embedding:
            input_data_emb = input_data* math.sqrt(self.ninp)
        else:
        ...
        ret = (sent_decoded_vocab, sent_decoded) if return_feature else sent_decoded_vocab

        return ret
        

    def forward_sent(self, input_dec, input_noise, gumbel_temp, is_embedding=True, return_feature=False):
        sent_embedding, encoder_out = self.forward_sent_encoder(input_noise, is_embedding)
        sent_decoded_vocab_soft = self.forward_sent_decoder(sent_embedding, input_dec, encoder_out, gumbel_temp, is_embedding, return_feature)
        return sent_decoded_vocab_soft

# max_T T(x,z)
# min_A [  (max_T T(x,z))   + reconst]