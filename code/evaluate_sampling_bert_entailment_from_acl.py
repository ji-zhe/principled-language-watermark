...
def evaluate(data_source, out_file, batch_size=10, on_train=False):
    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        with torch.no_grad():
            ...
            for beam in range(0,args.samples_num):
                ...
                nli_encodings = nli_tokenizer(orig_text, output_text, add_special_tokens=True,
                                               return_tensors='pt',
                                               padding='longest')
                nli_encodings = {k: v.cuda() for k, v in nli_encodings.items()}
                scores = nli_model(**nli_encodings).logits
                entail_score = torch.nn.functional.softmax(scores, dim=-1)[:, 2].item()
                entailment_score_beams.append(entail_score)
    ...
    return total_loss_lm/batch_count, correct_msg_count/tot_count, correct_msg_count_bits/tot_count_bits, meteor_tot/batch_count, l2_distances/batch_count, fake_correct/batch_count, real_correct/batch_count, Fscore ,  np.mean(p_value), p_value_smaller/len(p_value), entailment_score_tot/batch_count
...
nli_tokenizer = AutoTokenizer.from_pretrained('roberta-large')
...
val_lm_loss, val_correct_msg, val_correct_bits_msg,val_meteor, val_l2_sbert, val_correct_fake, val_correct_real, val_Fscore, val_pvalue, val_pvalue_inst, val_entailment = evaluate(val_data, f, eval_batch_size)

print('-' * 150)
print('| validation | lm loss {:5.2f} | msg accuracy {:5.2f} | msg bit accuracy {:5.2f} |  meteor {:5.4f} | SentBert dist. {:5.4f} | fake accuracy {:5.2f} | real accuracy {:5.2f} | F1 score {:.5f} | P-value {:.9f} | P-value inst {:5.2f} | entailment score: {:5.2f}'.format(val_lm_loss,val_correct_msg*100, val_correct_bits_msg*100, val_meteor, val_l2_sbert, val_correct_fake*100,val_correct_real*100,val_Fscore,val_pvalue,val_pvalue_inst*100, val_entailment))
print('-' * 150)
f.close()
...
test_lm_loss, test_correct_msg, test_correct_bits_msg, test_meteor, test_l2_sbert, test_correct_fake, test_correct_real, test_Fscore, test_pvalue, test_pvalue_inst,test_entailment = evaluate(test_data, f, test_batch_size)

print('=' * 150)
print('| test | lm loss {:5.2f} | msg accuracy {:5.2f} | msg bit accuracy {:5.2f} | meteor {:5.4f} | SentBert dist. {:5.4f} | fake accuracy {:5.2f} | real accuracy {:5.2f} | F1 score {:.5f} | P-value {:.9f} | P-value inst {:5.2f} | entailment score: {:5.2f}'.format(test_lm_loss, test_correct_msg*100, test_correct_bits_msg*100, test_meteor, test_l2_sbert, test_correct_fake*100,test_correct_real*100,test_Fscore,test_pvalue, test_pvalue_inst*100, test_entailment))
print('=' * 150)
f.close()
