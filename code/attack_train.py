...
def model_save(fn):
    with open(fn+'_mine_xy.pt', 'wb') as f:
        torch.save(mine_xy, f)
...
if args.atk_resume:
    with open(args.atk_resume+"_atk.pt", 'rb') as f:
        model_atk, criterion_atk, criterion_reconst, optimizer_atk = torch.load(f,map_location='cpu')
else:
    model_atk = attack_model.Attacker(ntokens,args.emsize,args.encoding_layers,args.dropout_transformer,args.dropouti, args.dropoute, args.tied,args.attn_heads,autoenc_model)
    # model_atk = torch.load('model1_denoise_autoenc_attack.pt')[0]
    t_net = attack_model.MI_seq2seq(ntokens,args.emsize,args.dropout_transformer,args.attn_heads,args.encoding_layers)
    mine_xy = mine.Mine(t_net,loss='mine')
    # mine_uy = Mine(T=attack_model.MI_feature2seqAndBits(args.emsize, args.dropout_transformer, args.attn_heads, args.encoding_layers, args.key_len), loss='mine',method=None)


if args.cuda:
    model_gen = model_gen.cuda()
    model_atk = model_atk.cuda()
    mine_xy = mine_xy.cuda()
    # mine_uy = mine_uy.cuda()
    # model_disc = model_disc.cuda()
    criterion = criterion.cuda()
    criterion_reconst = criterion_reconst.cuda()

###
params = list(model_gen.parameters()) + list(criterion.parameters()) + list(criterion_reconst.parameters()) + list(model_atk.parameters()) + list(mine_xy.parameters())
...
def train():
    ...
    while i < train_data.size(0) - 1 - 1:
        ...
        atk_data_emb, atk_one_hot, atk_data_prob = model_atk.forward_transformer(fake_one_hot.detach(), args.gumbel_temp,one_hot=True)
        # y = attacker(x,s) 
        # y = atk_star(x,s) = s         # msg_dec 乱猜

        # loss = distortion + msg_loss 
        # y是在x附近找一个msg loss较高的输出

        mine_xy.optimize(fake_one_hot.permute((1,0,2)).detach(), atk_one_hot.permute((1,0,2)).detach(), iters=5, batch_size=data_emb.size(1))
        freeze(mine_xy)
        ...
        loss_distor = loss_distor_fun(atk_data_prob, fake_one_hot.detach().view(-1,fake_one_hot.size(2)))
        # loss_distor = loss_distor_fun(atk_data_prob, fake_one_hot.argmax(dim=-1).view(-1))
        mi = -mine_xy(fake_one_hot.permute((1,0,2)), atk_one_hot.permute((1,0,2)))
        print("Loss of rec:", loss_msg.item(), "\tLoss of MI:", mi.item(), "\tLoss of distort:", loss_distor.item())
        # loss_atk = mi - loss_msg + args.dist_weight * ((loss_distor-args.distort)**2)
        loss_atk = args.dist_weight * ((loss_distor-args.distort)**2)
        loss_atk.backward()
        print("gradient: ")
