...
import os 
import sys
import model_mt_autoenc_cce_cgan_keyDiffLoc as model_mt_autoenc_cce_cgan
...
"""
CUDA_VISIBLE_DEVICES=0 nohup python main_train_cgan_kd_smine_keyDiffLoc.py --gan_D1_thres 1.0 --data data/wikitext-2 --emsize 512 --epochs 200 --batch_size 50 --bptt 80 --fixed_length 0 --save cgan_kd_smine_keyDiffLoc_kdData0924_WT2_T10_K32_BSZ50_new_phase2 --save_interval 20 --optimizer adam --msg_len 4 --use_semantic_loss 1 --sem_weight 6.0 --use_lm_loss 1 --lm_weight 2.0 --lm_ckpt /mnt/disk2/jizhe/zyc/awt/WT2_bert_lm.pt --use_reconst_loss 1 --discr_interval 1 --gen_weight 2.0 --msg_weight 3.0 --msg_label_weight 3.0 --reconst_weight 2.0 --xsmi_weight 1.0 --atk_mi_weight 1.0 --atk_reconst_weight 2.0 --atk_path /mnt/disk2/jizhe/zyc/awt/WT2_dae_bert.pt --atk_msg_weight 1 --pos_drop 0.1 --bert_kd --kd_alpha 0.25 --kd_beta 0.75 --saved_corpus /mnt/disk2/jizhe/zyc/awt/tokenizer_WT2.data --kd_data /mnt/disk2/jizhe/zyc/awt/kd/0924_WT2_T10_K32_BSZ50_new/ --resume cgan_kd_smine_keyDiffLoc_kdData0924_WT2_T10_K32_BSZ50_new > trainLog_cgan_kd_smine_keyDiffLoc_kdData0924_WT2_T10_K32_BSZ50_new_phase2.log &
"""
...
mine_uy = mine.Mine_withKey(model_mine.MI_3feat(args.emsize, args.dropout_transformer,args.attn_heads, args.encoding_layers), loss='mine').cuda()
mine_us = mine.Mine_withKey(model_mine.MI_3feat(args.emsize, args.dropout_transformer,args.attn_heads, args.encoding_layers), loss='mine').cuda()
###
if not criterion:
...
def bert_fill(data_one_hot, mask):
    data_ids = data_one_hot.argmax(dim=-1)
    data_ids[mask] = 103
    bert_logits = bert_masklm(data_ids)[0]
    return bert_logits.argmax(dim=-1)
...
def evaluate(data_source, batch_size=10, kd_prob_set=None, kd_idx_set=None):
    ...
    with torch.no_grad():
        for i in range(0, data_source.size(0) - args.bptt, args.bptt):
            ...
            msg_dec_input = atk_data_hot.view(fake_data_emb.shape[0],fake_data_emb.shape[1],-1) # torch.mm(atk_data_hot,model_gen.embeddings.weight).view(*fake_data_emb.shape)
            # msg_dec_input = torch.mm(atk_data_hot,model_gen.embeddings.weight).view(*fake_data_emb.shape)
            msg_out, labels_out = model_gen.forward_msg_decode(msg_dec_input, keys, bert_masklm)
            msg_out_2, labels_out_2 = model_gen.forward_msg_decode(fake_one_hot.view(fake_data_emb.shape[0], fake_data_emb.shape[1], -1), keys, bert_masklm)
            ...
            key_data_one_hot = msg_dec_input.clone() # atk_data_hot resized
            key_data_one_hot = bert_fill(key_data_one_hot, keys)
            key_data_emb = model_gen.forward_sent_encoder(key_data_one_hot, 0, 0, True)
            ...
            total_mine_us += -mine_us(fake_sent_msg_embedding, data_feat, key_data_feat)
            total_mine_uy += - mine_uy(fake_sent_msg_embedding,atk_data_feat, key_data_feat)
...
def train():
	...
	#get the embeddings from the generator network of the real 
        ...
        mine_uy.optimize(fake_sent_msg_embedding.detach(), atk_data_feat.detach(), key_data_feat.detach(), iters=2, batch_size=fake_data_emb.size(1),opt=optimizer_mine_uy) ### : batch first! 
        mine_us.optimize(fake_sent_msg_embedding.detach(), data_feat.detach(), key_data_feat.detach(), iters=2, batch_size=fake_data_emb.size(1),opt=optimizer_mine_us) ### : batch first! 
        ...
        errG_mi = -(mine_us(fake_sent_msg_embedding, data_feat,key_data_feat) - mine_uy(fake_sent_msg_embedding,atk_data_feat, key_data_feat) ) 

        data_emb = model_gen.forward_sent(data,msgs,args.gumbel_temp,only_embedding=True)
        msg_out_clean, labels_out_clean = model_gen.forward_msg_decode(fake_one_hot.view(fake_data_emb.shape[0],fake_data_emb.shape[1],-1), keys, bert_masklm)
