...
@torch.enable_grad()
def optimizing_attack(fake, batch_size, model_gen, n_steps, dist_weight=1.0):
    ...
    msg_atk = torch.randint(low=0,high=2,size=(batch_size, args.msg_len), dtype=torch.float).detach().cuda()
    msg_atk.requires_grad=True
    optimizer = torch.optim.Adam([atk_emb, msg_atk])
    ...
    for _ in range(n_steps):
        ...
        # import pdb;pdb.set_trace()
        total = loss_atk + dist_weight* loss_dist
        total.backward()
        optimizer.step()
        msg_atk.clip(0.0, 1.0)
...
def evaluate(data_source, out_file, batch_size=10, on_train=False):
    ...
    for i in range(0, data_source.size(0) - args.bptt, args.bptt):
        ...
        with torch.no_grad():
            ...
            data_adv_autoenc, msg_atk = optimizing_attack(best_beam_data,batch_size,model_gen,n_steps=1000,dist_weight=0)
