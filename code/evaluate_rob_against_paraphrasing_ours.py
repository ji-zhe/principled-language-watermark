...
def evaluate(data_source, out_file, batch_size=10, on_train=False):
    ...
    model_disc.eval()
    langModel.eval()
    paraphrased = corpus.gettokens_rebuttal('paraphrase_lines_indexed.txt')
    total_loss_lm = 0
    ntokens = len(corpus.dictionary)
    tot_count = 0
    correct_msg_count = 0
    correct_atk_msg_count_bits = 0
    tot_count_bits = 0
    ...
    long_msg_atk = np.zeros([batch_size, args.msg_len*args.msgs_segment])
    ...
    long_msg_atk = torch.from_numpy(long_msg_atk).float().cuda()
    long_msg_out_atk = np.zeros([batch_size, args.msg_len*args.msgs_segment])
    long_msg_out_atk = torch.from_numpy(long_msg_out_atk).float().cuda()
    p_value = []
    real_correct = 0
    fake_correct = 0 
    y_out = []
    y_label = []
    for batch_id, i in enumerate(range(0, data_source.size(0) - args.bptt, args.bptt)):
        if batch_id==26:
            break
        ...
        with torch.no_grad():
            ...
            print(i)			
            data_adv_autoenc = paraphrased[batch_id].unsqueeze(1).cuda()
            # import pdb; pdb.set_trace()
            ...
            msg_out,_ = model_gen.forward_msg_decode(data_dv_autoenc_emb.detach())
            msg_out_atk, _ = model_gen.forward_msg_decode(data_dv_autoenc_emb.detach())
            ...
            if bert_diff_selected < args.bert_threshold:
                ...
                output_list_adv = []
                for k in range(0, word_idx_adv.size(0)):
                ...
                long_msg_out_atk[:,long_msg_count*args.msg_len:long_msg_count*args.msg_len+args.msg_len] = msg_out_atk
                ...
                msg_out_random, _ =  model_gen.forward_msg_decode(data_emb)
                long_msg_out_atk[:,long_msg_count*args.msg_len:long_msg_count*args.msg_len+args.msg_len] = msg_out_random
        ...
        if batch_count != 0 and (batch_count+1)%args.msgs_segment==0:
            ...
            correct_atk_msg_count_bits = correct_atk_msg_count_bits + compare_msg_bits(long_msg_atk,long_msg_out)
            p_value.append(binom_test(similar_bits, all_bits, 0.5))
        else:
            long_msg_count = long_msg_count + 1
        correct_rebuttal += compare_msg_bits(msgs, msg_out)
        print(correct_rebuttal)
        print(msgs)
        total_rebuttal += torch.tensor(msgs.squeeze().size()).item()
    ...
    print(correct_rebuttal)
    print(total_rebuttal)
    print(correct_msg_count_bits)
    print(tot_count_bits)
    return total_loss_lm/batch_count, correct_msg_count/tot_count, correct_msg_count_bits/tot_count_bits, meteor_tot/batch_count, spacy_meteor_tot/batch_count, l2_distances/batch_count, fake_correct/batch_count, real_correct/batch_count, Fscore ,  np.mean(p_value), p_value_smaller/len(p_value), correct_atk_msg_count_bits/tot_count_bits
...
if args.eval_valid:
    ...
    val_lm_loss, val_correct_msg, val_correct_bits_msg,val_meteor,val_spacy_meteor, val_l2_sbert, val_correct_fake, val_correct_real, val_Fscore, val_pvalue, val_pvalue_inst, val_correct_bits_msg_atk = evaluate(val_data, f, eval_batch_size)

    print('-' * 150)
    print('| validation | lm loss {:5.2f} | msg accuracy {:5.2f} | msg bit accuracy {:5.2f} | msg bit accuracy of attacker {:5.2f}| meteor {:5.4f} | spacy meteor {:5.4f} | SentBert dist. {:5.4f} | fake accuracy {:5.2f} | real accuracy {:5.2f} | F1 score {:.5f} | P-value {:.9f} | P-value inst {:5.2f}'.format(val_lm_loss,val_correct_msg*100, val_correct_bits_msg*100, val_correct_bits_msg_atk*100, val_meteor, val_spacy_meteor, val_l2_sbert, val_correct_fake*100,val_correct_real*100,val_Fscore,val_pvalue,val_pvalue_inst*100))
    print('-' * 150)
    f.close()
    if args.count_metric:
        f_metrics.close()
...
test_lm_loss, test_correct_msg, test_correct_bits_msg, test_meteor, test_spacy_meteor, test_l2_sbert, test_correct_fake, test_correct_real, test_Fscore, test_pvalue, test_pvalue_inst, test_correct_bits_msg_atk = evaluate(test_data, f, test_batch_size)

print('=' * 150)
print('| test | lm loss {:5.2f} | msg accuracy {:5.2f} | msg bit accuracy {:5.2f} | msg bit accuracy of attacker {:5.2f}| meteor {:5.4f} | spacy meteor {:5.4f} | SentBert dist. {:5.4f} | fake accuracy {:5.2f} | real accuracy {:5.2f} | F1 score {:.5f} | P-value {:.9f} | P-value inst {:5.2f}'.format(test_lm_loss, test_correct_msg*100, test_correct_bits_msg*100, test_correct_bits_msg_atk*100, test_meteor, test_spacy_meteor, test_l2_sbert, test_correct_fake*100,test_correct_real*100,test_Fscore,test_pvalue, test_pvalue_inst*100))
print('=' * 150)
f.close()
if args.count_metric:
    f_metrics.close()
